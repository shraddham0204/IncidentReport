...
import { View, Text, TextInput, ScrollView, PixelRatio, Image, TouchableOpacity, AsyncStorage, NetInfo } from 'react-native';
...
class IncidentForm extends Component {
    constructor(props) {
        super(props);
        this.receiveLoginDetails = (value) => {
            console.log(value);
    ...
    componentDidMount() {
        ...
        navigator.geolocation.getCurrentPosition((position) => {
            ...
            fetch(`https://maps.googleapis.com/maps/api/geocode/json?latlng=${this.state.latitude},${this.state.longitude}&key=${GOOGLE_API}`)
                .then((res) => res.json())
                .then((json) => {
                if (json.status !== 'OK') {
    ...
    render() {
        return (<ScrollView style={{ backgroundColor: 'white' }}>
            ...
            <TextInput style={styles.inputStyle} placeholder='Briefly describe what happened including the sequence of events,conditions present at time of incident;who and what was involved.' autoCorrect={false} underlineColorAndroid='white' editable={true} maxLength={1400} multiline={true} numberOfLines={20} onChangeText={this._changeSummary.bind(this)} value={this.props.incidentSummary}/>
          </View>
          <View>
          <Input label=" Where did the incident occur?" placeholder="Provide the accurate location" onChangeText={this.changeLocation.bind(this)} value={this.state.place}/>
          </View>
          <View style={styles.dateContainerStyle}>  
          <Text style={styles.labelStyle}>When did the incident occur?</Text>
          <DatePicker style={styles.dateStyle} date={this.state.date} mode="date" placeholder="select date" format="MM-DD-YYYY" minDate="2016-05-01" maxDate={this.state.maxDate} confirmBtnText="Confirm" cancelBtnText="Cancel" customStyles={{
            dateIcon: {
                position: 'absolute',
                left: 0,
                top: 4,
        ...
        <DatePicker style={styles.dateStyle} date={this.state.time} mode="time" format="HH:mm" confirmBtnText="Confirm" cancelBtnText="Cancel" minuteInterval={10} onDateChange={(time) => { this.setState({ time: time }); }} customStyles={{
            dateIcon: {
                position: 'absolute',
                left: 0,
                top: 4,
