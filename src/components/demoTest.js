...
class demoTest extends Component {
    ...
    render() {
        ...
        <FlatList data={this.state.data} renderItem={({ item }) => (<ListItem roundAvatar title={item.l_incident} subtitle={item.l_incident} containerStyle={{ borderBottomWidth: 0 }}/>)} keyExtractor={item => item.l_incident} ItemSeparatorComponent={this.renderSeparator} ListHeaderComponent={this.renderHeader} ListFooterComponent={this.renderFooter} onRefresh={this.handleRefresh} refreshing={this.state.refreshing} onEndReached={this.handleLoadMore} onEndReachedThreshold={50}/>
      </List>);
    }
}
